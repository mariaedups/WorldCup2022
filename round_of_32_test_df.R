...
for(i in 1:nrow(matches_round_32)){

  team <- matches_round_32[i,]$opp_team
  group_team <- matches_round_32[i,]$group_opp_team
  rank_team <- matches_round_32[i,]$rank_opp
  opp_team <- matches_round_32[i,]$team
  group_opp_team <- matches_round_32[i,]$group_team
  rank_opp <- matches_round_32[i,]$rank_team
  match_num <- matches_round_32[i,]$match_num
  simulation <- matches_round_32[i,]$simulation
  matches_round_32 <- rbind(matches_round_32,cbind(team,group_team,rank_team,
...
df_team <- merge(matches_round_32,country_continent,by.x = 'team', by.y = 'country',all.x = TRUE) %>%
...
df_clean_round_32 <- merge_elo_all %>% mutate(year = 2026,
